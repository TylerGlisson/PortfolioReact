...
It is a pity, but markdown does **not** work in here for most markdown parsers.  [Marked] handles it pretty well.
  ...
  updateInput = (event) => {
    ...
    console.log(this.state.editorInput);
  }

  render() {
    let previewOutput = {__html: marked(this.state.editorInput)};
   
    return (
      <div className="markedDiv">
        <div className='editorDiv'>
          <textarea value={this.state.editorInput}  name="editor" id="editor" cols="1" onInput={this.updateInput}></textarea>
        </div>
        <div className='previewDiv'>
          <div className="preview" id="preview" dangerouslySetInnerHTML={previewOutput}></div>
        </div>
      </div>
    );
  }
